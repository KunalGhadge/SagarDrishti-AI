        ...
        <CardHeader className="shrink gap-y-0">
          <CardTitle className="flex gap-3 items-stretch min-w-0">
            <div
              style={{
                backgroundColor:
                  item.icon?.style?.backgroundColor || "rgb(14, 165, 233)",
              }}
              className="p-2 rounded-lg flex items-center justify-center ring ring-background border shrink-0 min-w-10 min-h-10 text-xl"
            >
              {type === "mcp" ? (
                <MCPIcon className="fill-white size-6" />
              ) : typeof item.icon?.value === "string" &&
                !item.icon.value.startsWith("http") ? (
