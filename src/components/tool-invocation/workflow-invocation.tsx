...
interface WorkflowInvocationProps {
  result?: VercelAIWorkflowToolStreamingResult | null;
}

function PureWorkflowInvocation({ result }: WorkflowInvocationProps) {
  const { copied, copy } = useCopy();
  const savedResult = useRef<VercelAIWorkflowToolStreamingResult | undefined | null>(result);
  ...
  return (
    <div className="w-full flex flex-col gap-1">
      {history.map((item, i) => {
        const itemResult = item.result || savedResult.current?.history?.[i]?.result;
        return (
          <NodeResultPopup
            key={item.id || i}
            disabled={!itemResult}
          ...
          >
            <div
              ...
              className={cn(
                ...
                !!itemResult && "cursor-pointer hover:bg-secondary",
              )}
            >
