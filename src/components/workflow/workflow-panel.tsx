          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div
                style={{
                  backgroundColor:
                    workflow.icon?.style?.backgroundColor || "rgb(14, 165, 233)",
                }}
                onClick={() => setIsEditing(true)}
                className="border transition-colors hover:bg-secondary! group items-center justify-center flex w-8 h-8 rounded-md ring ring-background hover:ring-ring shrink-0"
              >
                {typeof workflow.icon?.value === "string" &&
                !workflow.icon.value.startsWith("http") ? (
